...
function e(e, t, n, i, r, s, a, o, l, u, c, h, d, p, f, $, m, g, v, w, y, x, b, T, M) {
    ...
    this.preload = function (t, n, i) {
        ...
        A.children.length > 0 && e.currstate.from.name == M.PagesTypes.MENU ? A.addChildAt(C.render().$el, 1) : A.addChild(C.render().$el);
        e.isLoading = !0;
        e.resize();
        ...
        if (t && !C) {
            ...
            if (!e.data.sections.textures.loaded)
                return void this.preload(M.DataTypes.SECTIONS, e.data.sections.collection, function (n) {
            ...
            if (!e.data.projects.textures.loaded && (t.to.data.type == M.PagesTypes.WALL || t.to.data.type == M.PagesTypes.LEAF))
                return void this.preload(M.DataTypes.PROJECTS, e.data.projects.collection, function (n) {
            ...
            if (!e.data.contacts.textures.loaded && t.to.data.type == M.PagesTypes.CLEAF)
                return void this.preload(M.DataTypes.CONTACTS, e.data.contacts.collection, function (n) {
    ...
    this.navigate = function (t) {
        if (0 != t.value) {
            ...
            if (e.data.sections.textures.loaded && (i != M.PagesTypes.WALL && i != M.PagesTypes.LEAF || e.data.projects.textures.loaded))
                if (i == M.PagesTypes.INDEX && t.type == M.DirsTipes.VERT)
                    e.data.sections.controller.updatePage(t.value);
                else if (i == M.PagesTypes.WALL && t.type == M.DirsTipes.VERT)
                e.data.projects.controller.updateWall(t.value);
            else if (i == M.PagesTypes.LEAF)
                e.data.projects.controller.updateLeaf(t.value);
            else if (i == M.PagesTypes.INFOS && t.type == M.DirsTipes.VERT)
                e.data.infos.controller.updatePage(t.value);
            else if (i == M.PagesTypes.CLEAF)
                return
        }
    };
    ...
    this.addState = function (t, n) {
        ...
        A.addChild(k.$el);
        k.setState(t);
        t.to.data.type != M.PagesTypes.WALL && t.to.data.type != M.PagesTypes.LEAF ? D.start() : D.stop();
    ...
    this.getTweensIn = function (e, t) {
        var n = S.tweenIn;
        if (t) {
            e.to.data.type == M.PagesTypes.WALL && t.to.data.type == M.PagesTypes.WALL && (n = S.tweenInWithRows);
            e.to.data.type == M.PagesTypes.LEAF && t.to.data.type == M.PagesTypes.LEAF && (n = S.tweenInWithSlide);
            e.to.data.type == M.PagesTypes.WALL && t.to.data.type == M.PagesTypes.LEAF && (n = S.tweenInFromLeaf);
            t.to.data.type == M.PagesTypes.MENU && (n = S.tweenInFromMenu)
        }
        return [n.call(S), k.tweenIn()]
    };
    this.getTweensOut = function (t) {
        var n = S.tweenOut;
        t.to.data.type == M.PagesTypes.INDEX && e.currstate.to.data.type != M.PagesTypes.INDEX && (n = S.tweenOutToPage);
        t.to.data.type == M.PagesTypes.WALL && e.currstate.to.data.type == M.PagesTypes.WALL && (n = S.tweenOutWithRows);
        t.to.data.type == M.PagesTypes.WALL && e.currstate.to.data.type == M.PagesTypes.LEAF && (n = S.tweenOutToLeaf);
        t.to.data.type == M.PagesTypes.LEAF && e.currstate.to.data.type == M.PagesTypes.LEAF && (n = S.tweenOutWithSlide);
        e.currstate.to.data.type == M.PagesTypes.MENU && (n = S.tweenOutToMenu)
        return [n.call(S)]
    };
    this.needClear = function (e, t, n) {
        var i = !1;
        t.to.data.type == M.PagesTypes.WALL && e && e.isSideShow && (e.clearSides(), i = !0);
        return i
    };
    this.addOnTop = function (e, t) {
        ...
        if (t) {
            if (t.to.data.type != e.to.data.type)
                (e.from.data.type == M.PagesTypes.MENU || e.to.data.type == M.PagesTypes.LEAF || e.to.data.type == M.PagesTypes.WALL || e.to.data.type == M.PagesTypes.INFOS || e.to.data.type == M.PagesTypes.CLEAF) && (n = !1)
            else
                e.to.data.type == M.PagesTypes.LEAF && (n = !1)
        }
        return n
    };
